...
export function sectionsToIssues(sections, options = {}) {
    const { minLevel = 2, maxLevel = 3, labelPrefix = '' } = options;
    const issues = [];
    const processSection = (section, parentTitle) => {
        if (section.level >= minLevel && section.level <= maxLevel) {
            ...
            if (titleLower.includes('bug') || titleLower.includes('fix'))
                labels.push('bug');
            if (titleLower.includes('enhancement'))
...
export function formatIssueAsMarkdown(issue, index) {
    let output = `## Issue ${index + 1}: ${issue.title}\n\n`;
    output += `**Labels:** ${issue.labels.join(', ')}\n\n`;
    output += `### Body\n\n${issue.body}\n`;
    return output;
}
export function formatIssuesForGitHub(issues) {
